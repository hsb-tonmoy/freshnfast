...
<span class="font-playfair text-5xl font-semibold">You Might Also Like</span>
<hr class="h-[5px] w-[5%] bg-lightGreen mt-2 mb-12" />

<div class="w-full flex flex-wrap gap-x-8 gap-y-8 lg:gap-y-6">
	{#each items as item}
		<div class="item flex flex-col">
			<div class="image">
				<img src={item.img_url} alt={item.title} class="object-cover w-80 h-52 rounded-3xl" />
			</div>
			<div class="details flex items-center justify-between mt-4 px-2">
				<div class="flex flex-col">
					<span class="title font-semibold font-playfair text-lg">{item.title}</span>
					<span class="price text-neutral-500 text-sm">${item.price}</span>
				</div>
				<span
					on:click={() => {
						addToCart(item);
					}}
					class="block w-7 h-7 text-lightGreen cursor-pointer"
